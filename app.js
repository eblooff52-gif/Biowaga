...
const TIER_NEED = { music: 1, background: 1, link_custom: 1, link_youtube: 2, discord_id: 1, snow: 2 };
...
function renderDashboard() {
  ...
  if (hint) {
    hint.innerHTML = tl < 1
      ? 'Статус: <strong>Игрок</strong> — музыка и фон только с VIP. <a onclick="showPage(\'shop\')">Купить VIP</a>'
      : `Статус: <strong>${tierNames[tl]}</strong>`;
  }
  ...
  document.querySelectorAll('[data-vip-only]').forEach((el) => {
    ...
    el.querySelectorAll('input,textarea,button').forEach((inp) => { inp.disabled = locked; });
  });
...
function saveDashboard(e) {
  ...
  if (!canUse(user, 'music') && f.music.files[0]) {
    document.getElementById('dash-msg').innerHTML = '<div class="alert alert-error">Музыка только для VIP</div>';
    return;
  }
  if (!canUse(user, 'background') && f.background.files[0]) {
    document.getElementById('dash-msg').innerHTML = '<div class="alert alert-error">Фон только для VIP</div>';
    return;
  }
...
function renderProfile() {
  ...
  const hasMusic = canUse(p, 'music') && !!(p.music_url || p.music_title);
  const showBg = canUse(p, 'background') && !!p.background;
